...
if(empty($name)) {
  ...
  if($pageint > 1) {
    $lastpage = $pageint - 1;
    echo '    <div style="width: 90%; margin: auto;"><form method="post" action="./racerresults.php" class="inline"><input type="hidden" id="racer" name="racer" value="' . $name . '"><input type="hidden" id="page" name="page" value="' . $lastpage . '"><button type="submit" class="link-button-2" style="float: left;">Last Page</button></form>';
  } else {
    echo '    <div style="width: 90%; margin: auto;">';
  }
  if($lastrecord - $firstrecord == 19) {
    echo '<form method="post" action="./racerresults.php" class="inline"><input type="hidden" id="racer" name="racer" value="' . $name . '"><input type="hidden" id="page" name="page" value="' . $nextpage . '"><button type="submit" class="link-button-2" style="float: right;">Next Page</button></form></div>' . PHP_EOL;
  } else {
    echo '</div>' . PHP_EOL;
  }
  $st->close();
