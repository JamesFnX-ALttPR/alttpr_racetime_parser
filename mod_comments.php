...
while($row = $result->fetch_assoc()) {
  echo '      <tr><td style="text-align: center;">' . htmlentities($row['name']) . '</td><td style="text-align: center;">' . htmlentities($row['comments']) . '</td><td style="text-align: center;"><a target="_blank" href="' . $domain . '/races/' . $row['race_id'] . '">Permalink</td><td style="text-align: center;"><form action="' . $domain . '/drop_comment.php" method="post"><input type="hidden" id="commentid" name="commentid" value="' . $row['id'] . '"><input type="submit" value="Delete Comment"></form></td></tr>' . PHP_EOL;
}
echo "    </table><br>" . PHP_EOL;
$nextpage = $pageint + 1;
if($pageint > 1) {
  $lastpage = $pageint - 1;
  echo '    <div style="width: 90%; margin: auto;"><form method="post" action="' . $domain . '/mod_comments.php" class="inline"><input type="hidden" id="gamemode" name="gamemode" value="' . $mode . '"><input type="hidden" id="exclude" name="exclude" value="' . $exclude . '"><input type="hidden" id="page" name="page" value="' . $lastpage . '"><button type="submit" class="link-button-2" style="float: left;">Last Page</button></form>';
} else {
  echo '    <div style="width: 90%; margin: auto;">';
}
if($lastrecord - $firstrecord == 49) {
  echo '<form method="post" action="' . $domain . '/mod_comments.php" class="inline"><input type="hidden" id="gamemode" name="gamemode" value="' . $mode . '"><input type="hidden" id="exclude" name="exclude" value="' . $exclude . '"><input type="hidden" id="page" name="page" value="' . $nextpage . '"><button type="submit" class="link-button-2" style="float: right;">Next Page</button></form></div>' . PHP_EOL;
} else {
  echo '</div>' . PHP_EOL;
}
$stmt->close();
