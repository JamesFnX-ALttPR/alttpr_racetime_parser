...
if(validateTime($time) == 0) {
    ...
    if($rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO custom_times (seed_id, name, forfeit, realtime, ingametime, cr, comments, vod) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("issssiss", $raceid, $name, $forfeit, $time, $igt, $cr, $comments, $vod);
                $stmt->execute();
                $stmt->close();
                echo '<!DOCTYPE html>' . PHP_EOL;
                echo '<html>' . PHP_EOL;
